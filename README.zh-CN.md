...
- **话我帮你想，发送你来定** —— 判断卡底部有「**怎么回 ▾**」，**点一下卡片就地展开** 4 个不同话术的候选，**每条都标出 Jev 预测的"可能出现的结果"**（被安抚 / 顺利接住 / 觉得被敷衍 / 可能更不满）+ 风险，并自动把更可能有好结果的排在前面。**点击即填入 QQ 输入框**，是否发送永远由你决定。
...
- **卡片跟着 QQ 走** —— 卡片钉在 QQ 聊天窗口上，移动/缩放窗口时跟着一起走；非卡片区域仍可点击穿透，聊天照常操作。
